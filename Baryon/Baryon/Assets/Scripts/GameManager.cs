...
public class GameManager : MonoBehaviour
{
    ...
    private void logs(int won, int unknown)
    {
        
        var path = "statisticLog.txt";
        if (File.Exists(path))
        {
            StreamWriter logWriter = File.AppendText(path);
            logWriter.WriteLine("Won games: " + won + " Unknown intances: " + unknown);
            logWriter.Close();
        }
        else
        {
    ...
    timesplayed = 0;
        ...
        while (!player1Won && !player2Won)
        {
            ...
            if (Player1Points>4)
            {
                if (Quinten.IsPlayer1)
                {
                    
                    Quinten.GameOver(true);
                    

                }
                else
                {
        //            Debug.Log("Aw I Lost says Quinten");
            ...
            else if (Player2Points > 4)
            {
                ...
                if (!Quinten.IsPlayer1)
                {
             //       Debug.Log("Yay I win says quinten");
                    Quinten.GameOver(true);
